...
def BMI(Height,Weight):
    ...
    return round(Weight/(round(Height/100,2)**2),2)

#新增BMI節點:59
for i in range(len(train_data_x)):
    for j in range(48):
        Height = train_data_x[i][j][51]
        Weight = train_data_x[i][j][57]
        try:
            bmi = BMI(Height,Weight)
        except:
            bmi = 0
        train_data_x[i][j].append(bmi)
        train_data_x[i][j].pop(51)
        train_data_x[i][j].pop(56)
train_data_x = np.array(train_data_x)

for i in range(len(val_data_x)):
    for j in range(48):
        Height = val_data_x[i][j][51]
        Weight = val_data_x[i][j][57]
        try:
            bmi = BMI(Height,Weight)
        except:
            bmi = 0
        val_data_x[i][j].append(bmi)
        val_data_x[i][j].pop(51)
        val_data_x[i][j].pop(56)
...
57:BMI
'''
def BMI_max_min(data_x):
    #統計BMI的最大最小值
    bmi_max = -10000
    bmi_min = 10000
    for i in range(len(data_x)):
        for j in range(48):
            if data_x[i][j][57]<bmi_min:
                bmi_min = data_x[i][j][57]
            elif data_x[i][j][57]>bmi_max:
                bmi_max = data_x[i][j][57]
    return (bmi_max,bmi_min)
#%%
'''
BMI_max_min(train_data_x)=(6266.27,0.0)
BMI_max_min(val_data_x)=(170.11,0.0)
bmi_max = 6266.27
bmi_min = 0.0
'''
'''
...
57:BMI
'''
cont_channels=[2,3,49,50,51,52,53,54,55,56,57]#共12個
...
#處理完極端值，存檔
import pickle
with open("discrete_train_data.pickle", "wb") as fp:   #Pickling
    pickle.dump(train_data, fp)
with open("discrete_val_data.pickle", "wb") as fp:   #Pickling
