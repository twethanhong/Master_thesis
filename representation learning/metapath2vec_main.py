...
from metapath2vec_skipgram import build_model,traning_op,train
from metapath2vec_dataset import Dataset
...
def parse_args():
    ...
    parser.add_argument('--walks',type=str,default="C://Users//KDD204//Desktop//異構網路嵌入式學習//異構資料//Win="+win+"//win"+win+"_"+meta+".txt",help='text file that has a random walk in each line. A random walk is just a seaquence of node ids separated by a space.')
    parser.add_argument('--type1',type=str,default="C://Users//KDD204//Desktop//異構網路嵌入式學習//異構資料//id_labevent.txt",help='text file that has node types. each line is "node id <space> node type"')
    parser.add_argument('--type2',type=str,default="C://Users//KDD204//Desktop//異構網路嵌入式學習//異構資料//id_chartevent.txt",help='text file that has node types. each line is "node id <space> node type"')
    parser.add_argument('--epochs',type=int,default=10, help='number of epochs')
    # parser.add_argument('--batch',type=int,default=1, help='Batch size.Only batch one is supported now...')
    parser.add_argument('--lr',type=float,default=0.01, help='learning rate')
    parser.add_argument('--log',default="./win"+win+"_"+meta+"_log",type=str,help='log directory')
    ...
    parser.add_argument('--window',default=2,type=int,help='context window size')
...
def main(args):
    ...
    dataset=Dataset(random_walk_txt=args.walks,node_type_mapping_txt=args.type1,node_type_mapping_txt_1=args.type2,node_type="labevent",node_type_1="chartevent",window_size=args.window)
